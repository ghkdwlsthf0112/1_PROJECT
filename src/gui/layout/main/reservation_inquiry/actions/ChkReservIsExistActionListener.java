...
//	messageBox(this, "아이디를 입력한 뒤 중복확인을 해주세요.");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == ReservationOkBtn && mainFrame.getTitle().equals("예약체크인")) {
			// 예약 번호
			...
			if (reservationDao.getChkInStartDate(reservationNumber) == null) {
				...
				System.out.println(chkInStartDate+"\t"+currentDate);
				// 예약 시작 날짜와 현재 날짜 비교
				if (chkInStartDate.equals(currentDate)) {
					if (reservationDao.chkReservation(displayField.getText()) == 1
							&& mainFrame.getTitle().equals("예약확인")) {
						new ReservationInfo(reservationDao.getReservstion(displayField.getText()));
						mainFrame.dispose();
					} else if (reservationDao.chkReservation(displayField.getText()) == 1
							&& mainFrame.getTitle().equals("예약체크인")) {
					...
					info.showMessageDialog(mainFrame, "당일 외에는 체크인이 불가능합니다.", "Message",
